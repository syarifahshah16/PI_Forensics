...
def selectCredentialsEvidenceDetails(conn, id):
    cursor = conn.cursor()
    cursor.execute('''SELECT frameNum, srcHost, dstHost FROM credentialsEvidenceTable WHERE id=?''', (id,)) # note trailing comma as we're passing a tuple with a single value
    row = cursor.fetchone()
    return row