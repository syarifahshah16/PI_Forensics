...
import os
import random
import summaryTab, newCaseDialog, hdAnalyzedDataTab, hdDeletedFilesTab, mainmenu, search, searchTab
import connectdb
import subprocess
import sqlite3
...
class mainFrame(wx.Frame):
    ...
    def addAuiTab(self, tabName, evidenceDetails):
        ...
        if tabName == "Deleted files":
            ...
            self.auiNotebook.AddPage(hdDeletedFilesTab.TabPanel(self.auiNotebook, tabName, caseDir), tabName, False, wx.NullBitmap) #calls and open a aui tab from DeletedFilesTab.py
        ...
        if tabName == "Bookmarks":
            ...
            self.auiNotebook.AddPage(hdAnalyzedDataTab.TabPanel(self.auiNotebook, tabName, evidenceDetails, caseDir, caseDbPath), tabName, False, wx.NullBitmap)  #calls and open a aui tab from SummaryTab.py
        ...
        for x in analyzedDataTree:
            if tabName == x and tabName != "Deleted files":
                self._dialog = wx.ProgressDialog("Loading", "Loading {tabName}".format(tabName=tabName), 100)
                LoadingDialog(self._dialog)
                addingPage = self.auiNotebook.AddPage(hdAnalyzedDataTab.TabPanel(self.auiNotebook, tabName, evidenceDetails, caseDir, caseDbPath), tabName, False, wx.NullBitmap)
                LoadingDialog.endLoadingDialog(self)
        ...
        for x in documentsTree:                                                                                                            
            if tabName == x:
                self._dialog = wx.ProgressDialog("Loading", "Loading {tabName}".format(tabName=tabName), 100)
                LoadingDialog(self._dialog)                                                                        
                self.auiNotebook.AddPage(hdAnalyzedDataTab.TabPanel(self.auiNotebook, tabName, evidenceDetails, caseDir, caseDbPath), tabName, False, wx.NullBitmap)
                LoadingDialog.endLoadingDialog(self)
