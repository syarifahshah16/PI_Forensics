...
class mainFrame(wx.Frame):
    ...
    def onAddEvidence(self, event):
        try:
            ...
            # creating thread 
            t1 = threading.Thread(target=mainFrame.runddfile, args=(lock,))
    ...
    def onSearchBtn(self, event):
        try:
            ...
            if searchItem[1] == "Normal Search":
                ...
                mainFrame._dialog = wx.ProgressDialog("Search", "Searching for {val}".format(val=searchItem[0]), 100)
                LoadingDialog(mainFrame._dialog)
...
class LoadingDialog():
    def __init__(self, _dialog):
        mainFrame._dialog = _dialog
        mainFrame._dialog.Center()
        mainFrame._dialog.Pulse()
        self.run()
     
    def run(self):
        count = 0
        while True:
            mainFrame._dialog.Update(count)
            if count == 100:
                break
            count += 2
        
    def endLoadingDialog(self):
        mainFrame._dialog.Destroy()
