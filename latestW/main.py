...
class mainFrame(wx.Frame):
    ...
    def onAddEvidence(self, event):
        ...
        try:
            ...
            self.runFiles(evidencePath, openFileDialog)
            self.runSessions(evidencePath, openFileDialog)  



    def runFiles(self, evidencePath, openFileDialog):
        """relating to extraction of values from PCAP file"""
        global caseDbPath
    ...
    def runSessions(self, evidencePath, openFileDialog):
        #lock.acquire()
        ...
        if (None == caseDbPath):
            print("Error: runSessions - global caseDbPath invalid!")
            return False     """           
       ...
       #addEvidenceDbConn = connectdb.create_connection(caseDbPath)
        ...
        for timestamp, buf in pcap:
            ...
            SessionsTab.SessionsTabPanel.addSessionsDetails(window, sequence)
        #lock.release()
    ...
    def addAuiTab(self, tabName, evidenceDetails):
        ...
        if tabName == "Sessions":
            self._dialog = wx.ProgressDialog("Loading", "Loading {tabName}".format(tabName=tabName), 100)  #create loading dialog
            LoadingDialog(self._dialog)                                                                    #start loading 
            self.auiNotebook.AddPage(SessionsTab.SessionsTabPanel(self.auiNotebook, caseDir), tabName, False, wx.NullBitmap) #calls and open a aui tab from DeletedFilesTab.py
            LoadingDialog.endLoadingDialog(self)    
