    ...
    def onAddEvidence(self, event):
        ...
        try:
            ...
            for timestamp, buf in pcap:
                ...
                row = [int(Packet), str(datetime.datetime.utcfromtimestamp(timestamp)), str(src_ip), str(dst_ip), str(repr(request))]
                mainNetAnalysis.addPCAPDetails(self, row)
                connectdb.insertPcapEvidenceDetails(self.conn, int(Packet), str(datetime.datetime.utcfromtimestamp(timestamp)), str(src_ip), str(dst_ip), str(request)) 
                #self.evidenceDetails = 1 #to flag that we've got evidence
