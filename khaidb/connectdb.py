...
def createPcapSessionsTable(conn):
    try:
        cursor = conn.cursor() # Get a cursor object
        cursor.execute('''CREATE TABLE pcapSessionsTable(id INTEGER PRIMARY KEY, Packet INTEGER, timestamp TEXT, src_ip TEXT, dst_ip TEXT, request TEXT''')
        conn.commit()
        
    except Error as e:
        print(e)

def insertPcapSessionsDetails(conn, frameNum, filePath, srcHost, srcPort, dstHost, dstPort, protocol, filename, ext, size, timestamp):
    cursor = conn.cursor()
    cursor.execute('''INSERT INTO pcapEvidenceTable(Packet, timestamp, src_ip, dst_ip, request) VALUES(?,?,?,?,?)''',
