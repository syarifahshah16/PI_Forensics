...
class mainFrame(wx.Frame):
        ...
        for ts, buf in pcap:
            ...
            if eth.type == dpkt.ethernet.ETH_TYPE_IP:
                ...
                if (src_port == 80) or (src_port == 443) or (dst_port == 80) or (dst_port == 443): # only show packets that are to do with file transfers (via port 80 or port 443)
                    ...
                    dbPreBufList.append( (int(frameNumber), str(evidencePath), str(src_host_str), str(src_port), str(dst_host_str), str(dst_port), str(protocol), str(fileName), str(ext), str(size), str(timestamp) ) ) # list of tuples
    ...
    def onAddSessionsEvidence(self, evidencePath, openFileDialog):
        ...
        for timestamp, buf in pcap:
            ...
            self.evidenceDetails = 1 # flag that we've got evidence
            sessionsPreBufList.append( (int(Packet), str(timestamp), str(src_ip), str(dst_ip), str(request) ) ) # list of tuples
    ...
    def addAuiTab(self, tabName, evidenceDetails):
        ...
        if tabName == "File":
            ...
            global fileCentric
            _thread.start_new_thread(self.pcapDataDbToGui, (window, fileCentric,) )
        ...
        if tabName == "Sessions":
            self._dialog = wx.ProgressDialog("Loading", "Loading {tabName}".format(tabName=tabName), 100)  #create loading dialog
            LoadingDialog(self._dialog)                                                                    #start loading 
            self.auiNotebook.AddPage(SessionsTab.SessionsTabPanel(self.auiNotebook, tabName, caseDir), tabName, False, wx.NullBitmap) #calls and open a aui tab from DeletedFilesTab.py
            LoadingDialog.endLoadingDialog(self)    
