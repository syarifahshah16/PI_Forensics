...
class mainFrame(wx.Frame):
    ...
    def recreateTree(self, caseDbFile):
        ...
        self.tree_ctrl_1.AppendItem(root, "Files")
    ...
    def onAddEvidence(self, event):
        ...
        try:
            ...
            t1 = threading.Thread(target=mainFrame.onAddSessionsEvidence, args = (self, evidencePath, openFileDialog,))
            t2 = threading.Thread(target=mainFrame.onAddDNSEvidence, args = (self, evidencePath, openFileDialog,))
            t3 = threading.Thread(target=mainFrame.onAddFilesEvidence, args = (self, evidencePath, openFileDialog,))
            t4 = threading.Thread(target=mainFrame.onAddImagesEvidence, args = (self, evidencePath, openFileDialog,))
            t5 = threading.Thread(target=mainFrame.onAddCredentialsEvidence, args = (self, evidencePath, openFileDialog,))
    ...
    #----------------------------#   
    def onAddFilesEvidence(self, evidencePath, openFileDialog):
        #lock.acquire()
        ...
        if (None == caseDbPath):
            print("Error: onAddFilesEvidence - global caseDbPath invalid!")
            return False
    ...
    def onAddImagesEvidence(self, evidencePath, openFileDialog):
        #lock.acquire()
        #global caseDbPath, evidencePath, openFileDialog
        ...
        if (None == caseDbPath):
            print("Error: onAddImagesEvidence - global caseDbPath invalid!")
            return False
        ...
        for ts, buf in pcap:
            ...
            if eth.type == dpkt.ethernet.ETH_TYPE_IP:
                ...
                if (src_port == 80) or (src_port == 443) or (dst_port == 80) or (dst_port == 443): 
                    ...
                    #append extracted data to the temporary buffer so that we can separate PCAP extraction from database interaction
                    #list of tuples
                    imagesPreBufList.append( (str(frameNumber), str(fileName), str(ext), str(size), str(evidencePath), str(src_host_str), str(src_port), str(dst_host_str), str(dst_port) ) ) 
        ...
        for i in range (0, numRows, 20): 
            ...
            for j in range (i, i + 20): 
                ...
                #row tuple
                r = imagesPreBufList[j] 

                #insert values into table
                cursor.execute('''INSERT INTO imagesEvidenceTable(frameNum, imageName, ext, size, filePath, srcHost, srcPort, dstHost, dstPort) VALUES(?,?,?,?,?,?,?,?,?)''',
    ...
    def onAddCredentialsEvidence(self, evidencePath, openFileDialog):
        ...
        if (None == caseDbPath):
            print("Error: onAddCredentialsEvidence - global caseDbPath invalid!")
            return False
        ...
        for ts, buf in pcap:
            ...
            if eth.type == dpkt.ethernet.ETH_TYPE_IP:
                ...
                if (src_port == 80) or (src_port == 443) or (dst_port == 80) or (dst_port == 443): 
                    ...
                    #append extracted data to the temporary buffer so that we can separate PCAP extraction from database interaction
                    #list of tuples
                    credentialsPreBufList.append( (str(frameNumber), str(src_host_str), str(dst_host_str), str(protocol), str(username), str(password), str(vlogin), str(timestamp) ) ) 
        ...
        #transfer data (tuples) from buffer to database
        #http://specminor.org/2017/01/09/improve-sqlite-write-speed-python.html
        numRows = len(credentialsPreBufList)
        #print("Rows: " + str(numRows))
        ...
        for i in range (0, numRows, 20): 
            ...
            for j in range (i, i + 20): 
                ...
                #row tuple
                r = credentialsPreBufList[j] 

                #insert values into table
                cursor.execute('''INSERT INTO credentialsEvidenceTable(frameNum, srcHost, dstHost, protocol, username, password, vlogin, timestamp) VALUES(?,?,?,?,?,?,?,?)''',
    ...
    def addAuiTab(self, tabName, evidenceDetails):
        ...
        if tabName == "Images":
            ...
            self.auiNotebook.AddPage(ImagesTab.ImagesTabPanel(self.auiNotebook, tabName, caseDir, caseDbPath, caseDir), tabName, False, wx.NullBitmap) 
            LoadingDialog.endLoadingDialog(self)    

            #just added a page so the page we want to access is the last one
            window = self.auiNotebook.GetPage(self.auiNotebook.GetPageCount() - 1) 

            global imagesCentric
            #packet-centric output
            if(False == imagesCentric) : 
                #get the PCAP data from the database and display in the GUI (Files Tab)
                index = 1
                #if click packet-centric button for example [TODO]
                while (True):
                    #connect and select evidence
                    row = connectdb.selectImagesEvidenceDetails(self.conn, index)
                    if ( () == row or None == row ):
                        #from while-loop (no more data)
                        break 
                    
                    #post to sequence in pcapFilesTab.py    
                    ImagesTab.ImagesTabPanel.addImagesDetails(window, row)
                    index = index + 1
            ...
            #file-centric output  
                ...
                while (True):
                    #connect and select evidence
                    rowTuple = connectdb.selectImagesEvidenceDetails(self.conn, index)
                    ...
                    if (int(rowTuple[destPortCol]) == 80) or (int(rowTuple[destPortCol]) == 443): 
                        ...
                        if (rowTuple[filenameCol] != ""): 
                            ...
                            #look through next 10 rows for file size info
                            #each row corresponds to a tcp packet, so there are several packets per file transfer
                            if (rowTuple[extCol] == 'png') or (rowTuple[extCol] == 'jpg') or (rowTuple[extCol] == 'jpeg') or (rowTuple[extCol] == 'exif') or (rowTuple[extCol] == 'tiff') or (rowTuple[extCol] == 'gif') or (rowTuple[extCol] == 'bmp') or (rowTuple[extCol] == 'bpg'):
                                for j in range(index + 1, index +10): 
                                    #fileSize not in web client to server, but is in the packets passing on the other direction
                                    rowForSizeTuple = connectdb.selectImagesEvidenceDetails(self.conn, j)
                                ...
                                rowTuple = (fileIndex,) + rowTuple[1:] 
                                fileIndex = fileIndex + 1
                                ImagesTab.ImagesTabPanel.addImagesDetails(window, rowTuple)
        ...
        if tabName == "DNS":
            ...
            self.auiNotebook.AddPage(DNSTab.DNSTabPanel(self.auiNotebook, caseDir), tabName, False, wx.NullBitmap) 
            LoadingDialog.endLoadingDialog(self)    
        ...
        if tabName == "Credentials":
            ...
            LoadingDialog(self._dialog)     
            #calls and open a aui tab from pcapDNSTab.py
            self.auiNotebook.AddPage(CredentialsTab.CredentialsTabPanel(self.auiNotebook, tabName, caseDir), tabName, False, wx.NullBitmap) 
            LoadingDialog.endLoadingDialog(self)    
