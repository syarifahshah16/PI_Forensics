...
#   IMAGES DATABASE   #
#----------0----------#
def createImagesEvidenceTable(conn):
    try:
        cursor = conn.cursor() # Get a cursor object
        cursor.execute('''CREATE TABLE imagesEvidenceTable(id INTEGER PRIMARY KEY, frameNum TEXT, imageName TEXT, ext TEXT, size TEXT, filePath TEXT, srcHost TEXT, srcPort TEXT, dstHost TEXT, dstPort TEXT)''')
        conn.commit()
...
def selectImagesEvidenceDetails(conn, id):
    cursor = conn.cursor()
    #note trailing comma as we're passing a tuple with a single value
    cursor.execute('''SELECT frameNum, imageName, ext, size, filePath, srcHost, srcPort, dstHost, dstPort  FROM imagesEvidenceTable WHERE id=?''', (id,)) 
    row = cursor.fetchone()
    return row
...
def createCredentialsEvidenceTable(conn):
    try:
        cursor = conn.cursor() # Get a cursor object
        cursor.execute('''CREATE TABLE credentialsEvidenceTable(id INTEGER PRIMARY KEY, frameNum TEXT, srcHost TEXT, dstHost TEXT, protocol TEXT, username TEXT, password TEXT, vlogin TEXT, timestamp TEXT)''')
        conn.commit()
...
def selectCredentialsEvidenceDetails(conn, id):
    cursor = conn.cursor()
    cursor.execute('''SELECT frameNum, srcHost, dstHost, protocol, username, password, vlogin, timestamp FROM credentialsEvidenceTable WHERE id=?''', (id,)) 
    row = cursor.fetchone()
    return row